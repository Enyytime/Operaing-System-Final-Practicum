...
void cp(byte cwd, char* src, char* dst) {
    ...
    // Read file system nodes from disk into file_metadata_buf
    ...
    // Determine the parent index and filename of the destination
    if (dst[0] == '/') {
        // Absolute path
        dst_parent_index = FS_NODE_P_ROOT;
        ...
        // Relative path or subdirectory
        dst_parent_index = cwd;
        for (i = strlen(dst) - 1; i >= 0; i--) {
            if (dst[i] == '/') {
                dst[i] = '\0'; // Split into directory and filename
                strcpy(dst_name, dst + i + 1); // Filename
                break;
            }
        }
        if (i >= 0) {
            // Directory part exists, find the directory node
            for (i = 0; i < FS_MAX_NODE; i++) {
            ...
            // No directory part, use the whole dst as filename
            strcpy(dst_name, dst);
        }
    }
...
void cat(byte cwd, char* filename) {
    ...
    for (i = 0; i < FS_MAX_NODE; i++) {
        if (strcmp(node_fs_buf.nodes[i].node_name, filename) == 0 && node_fs_buf.nodes[i].parent_index == cwd) {
            file_index = i;
            break;
        }
    }
